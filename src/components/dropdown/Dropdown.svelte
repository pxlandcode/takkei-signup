...
<div class={`relative flex w-full flex-col gap-1 ${extraWrapperClasses}`}>
	<label for={id} class="pointer-events-none block text-base font-medium text-white">{label}</label>

	<button
		type="button"
		{id}
		class={`flex w-full flex-row content-center items-center justify-between rounded border px-3 py-2 text-left focus:outline-blue-500 
